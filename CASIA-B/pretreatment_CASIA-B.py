...
def imgs_to_pickle(pkl_name, INPUT_PATH, OUTPUT_PATH, save_cut_img=False, pixel_threshold=800):
    print(f"\t Walk to {INPUT_PATH}.")
    print(f"\t {save_cut_img=} {pixel_threshold=}")
    silhouette_dir = INPUT_PATH
    out_dir = OUTPUT_PATH
    all_imgs_pkl = os.path.join(out_dir, '{}.pkl'.format(pkl_name))
    ...
    if count_frame < 5:
        print('\t Seq:{}, less than 5 valid data.'.format(silhouette_dir))
    ...
    for _id in id_list:
        ...
        for _seq_type in seq_type[::1]:
            ...
            for _view in view[::1]:
                ...
                OUTPUT_PATH = os.path.join(output_path, _id, _seq_type, _view)
                imgs_to_pickle(_view, INPUT_PATH, OUTPUT_PATH)
