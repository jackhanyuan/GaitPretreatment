...
def imgs_to_pickle(pkl_name, silhouette_path, pkl_path, silhouette_cut_path=None, img_size=64, clean=True, augment=True, pixel_threshold=0):
    print()
    print(f"\t Walk to {silhouette_path}.")
    print(f"\t {img_size=} {clean=} {augment=} {pixel_threshold=}")

    all_imgs_pkl = os.path.join(pkl_path, '{}.pkl'.format(pkl_name))
    ...
    if count_frame < 5:
        print('\t Seq:{}, less than 5 valid data.'.format(silhouette_path))
...
def datasets_to_pkl(silhouette_folder, pkl_folder, silhouette_cut_folder=None, img_size=64, clean=True, augment=True, pixel_threshold=0):
    ...
    for _id in id_list:
        ...
        for _seq_type in seq_type[::1]:
            view = os.listdir(os.path.join(silhouette_folder, _id, _seq_type))
            view.sort()
            for _view in view[::1]:
